...
const Head = styled.header`
  ...
  align-items: center;
  background-color: ${props => props.bgColor ? props.bgColor : theme.colors.darkPurple };
  padding-left: 1rem;
  padding-right: 1rem;
...
const HeaderSubCopy = styled.h5`
  font-size: 20px;
  color: ${theme.colors.darkGrayLighter};
  max-width: 480px;
  margin-bottom: 10px;
...
const Header = ({ data, bgColor }) => {
  ...
  const three = () => <HeaderCopy style={{ transitionDelay: '300ms' }}>{data.copy} <HeaderLink href="https://digitalinnovation.one" target="_blank"> {data.company}</HeaderLink></HeaderCopy>;
